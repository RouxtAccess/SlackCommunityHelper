...
class ChannelViewModal {
    ...
    public function generateView(User $user)
    {
        $payload =  [
            'type' => 'modal',
            'private_metadata' => SlackConstants::VIEW_ID_MESSAGE_RULE_VIEW_CHANNEL,
            'title' => [
                'type' => 'plain_text',
                'text' => 'Message Rules',
            ],
        ...
        foreach (ChannelRule::all() as $rule)
        {
            $payload['blocks'][] =
                [
                    'type' => 'section',
                    ...
                    'accessory' =>
                        [
                            'type' => 'button',
                            'action_id' => SlackConstants::ACTIONS_MESSAGE_RULE_CHANNEL_DELETE,
                            'style' => 'danger',
                            ...
                            'confirm' =>
                                [
                                    'title' =>
                                        [
                                            'type' => 'plain_text',
                                            'text' => "Delete Channel Rule {$rule->getKey()}",
                                        ],
                                    'text' =>
                                        [
                                            'type' => 'plain_text',
                                            'text' => 'This will remove the Channel Rule',
                                        ],
                                    'confirm' =>
                                        [
                                            'type' => 'plain_text',
        ...
        if(count($payload['blocks']) === 2)
        {
            $payload['blocks'][] = [
                ...
                'text' =>
                    [
                        'type' => 'mrkdwn',
                        'text' => ':robot_face: No Channel Rules are currently active!',
                    ]
            ];
        }
        return $payload;
    ...
    public function handleDelete(User $user, object $payload)
    {
        ...
        $rule = ChannelRule::find($payload->actions[0]?->value);
        if(!$rule)
            return response('success', 200);

        Log::info('ChannelViewModal - Deleting ChannelRule', ['rule' => $rule->getKey(), 'user' => $payload->user->id]);
        $rule->delete();
